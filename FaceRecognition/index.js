...
Webcam.capture("./Data/tmp.JPG", function (err, data) { 
    (client.face.detect({
        ...
        client.face.similar(faceId,{candidateFaces: Linq.from(users).select(function (x) { return x.guids }).selectMany(function(x) {return x}).toArray()
        }).then(function (response)
        {
            console.log(response);
